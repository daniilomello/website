...
1. When creating an image in the Dockerfile you must start from an existing image using `FROM` to define it.
2. To run commands you must use `RUN`.
3. To define and create the work folder inside the container use `WORKDIR`
4. To copy the local folder into the container, use `COPY`
5. To define variable and fixed commands, you can use `CMD` for variable and `ENTRYPOINT` for fixed.
